...
- [ ] В `README.md` появилась секция `## Целевая система и тюнинг под неё` между `## Требования` и `## Установка` с описанием выше (hardware + блок «Если ваша система слабее» с `.env`-примером).
- [ ] Значения в примере для слабой системы согласованы с реальными ключами `.env.example` (имена ключей существуют).
- [ ] **Документация обновлена**: `README.md`. **Тесты**: `n/a`.
- [ ] `git status` чист; коммит `docs(readme): секция о целевой системе и тюнинге под слабые системы`.
...
- [ ] В `README.md` и `_docs/README.md` слово исправлено на «мульти-модельность» (опечаток «моделность» не осталось — проверить `grep -rn "моделность" README.md _docs`).
- [ ] В обоих файлах добавлен отдельный пункт «Мультимодальность» (текст/документ/голос/изображение).
- [ ] Абзац `## Возможности` в `README.md` упоминает спринты 10 и 11.
- [ ] **Документация обновлена**: `README.md`, `_docs/README.md`. **Тесты**: `n/a`.
- [ ] `git status` чист; коммит `docs: терминология (модельность/мультимодальность) и актуализация README под спринты 10–11`.
...
- [ ] В `_docs/stack.md` §13, `_docs/current-state.md` §3 и `AGENTS.md` (Навигация) есть лаконичный пункт о целевой системе со ссылкой на секцию README.
- [ ] Формулировки лаконичные (1–3 строки), без дублирования полной hardware-секции.
- [ ] **Документация обновлена**: `AGENTS.md`, `_docs/current-state.md`, `_docs/stack.md`. **Тесты**: `n/a`.
- [ ] `git status` чист; коммит `docs: зафиксировать целевую систему для code-агентов (AGENTS.md, stack, current-state)`.
...
- [ ] Шаги 1–4 дают ожидаемый результат (без рассинхронизаций).
- [ ] Шаг 5: ключи спринта 11 присутствуют в `stack.md` §9; пробелы (если найдены) закрыты.
- [ ] **Документация обновлена**: при необходимости. **Тесты**: `n/a`.
- [ ] `git status` чист; если были правки — коммит `docs: финальная сверка документации спринта 11`; если правок нет — отметить в Истории «сверка пройдена, правок не потребовалось».
