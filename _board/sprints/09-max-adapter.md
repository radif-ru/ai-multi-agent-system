...
- [ ] README не называет проект исключительно «Telegram-ботом»; упомянуты оба адаптера и реализованный multi-agent (проверка инспекцией).
- [ ] `grep -rn "pp/skills" README.md` — пусто; ссылка ведёт на `./app/skills`.
- [ ] Список возможностей/спринтов соответствует `_docs/current-state.md` (инспекция).
- [ ] **Документация обновлена** — да (это документационная задача).
- [ ] **Тесты добавлены / обновлены** — `n/a` (только `README.md`).
...
- [ ] Найденные расхождения README/доков с кодом по 01–08 устранены (инспекция); если расхождений нет — зафиксировать это в истории спринта.
- [ ] `grep` по ключевым ссылкам не находит «битых» путей в живых документах.
- [ ] **Документация обновлена** — да.
- [ ] **Тесты добавлены / обновлены** — `n/a` (только `_docs/`).
- [ ] `git status` чист.
...
- [ ] `Settings` подхватывает `MAX_*` из окружения; валидация не падает при пустом `MAX_BOT_TOKEN` (канал просто не запускается).
- [ ] `tests/users/test_repository.py` покрывает `get_or_create("max", ...)` → `channel="max"`, стабильный `user.id` между вызовами.
- [ ] `.env.example` дополнен; секрет не хардкодится.
- [ ] **Документация обновлена** — `_docs/stack.md` §9 (поля `MAX_*`).
...
- [ ] `tests/adapters/max/test_client.py` мокает `httpx` (без сети): `get_me`, `get_updates` (с `marker`), `send_message`; ошибки сети/таймаут → собственные исключения.
- [ ] Тест проверяет, что токен **не** попадает в текст лога (через `mask_secrets`).
- [ ] **Документация обновлена** — упоминание клиента в `_docs/architecture.md` §3.x (черновик, финал — в Этапе 5).
...
- [ ] Smoke: `python -c "from app.max_main import main; print(main)"` и `python -c "import app.adapters.max.adapter"` проходят.
- [ ] `tests/test_max_main.py` патчит сетевую часть (polling) одной точкой (как `tests/test_main.py`): сборка и shutdown отрабатывают без сети.
- [ ] Диспетчер маршрутизирует апдейт по типу (текст / команда / вложение) — заглушки для текста/команд/файлов закрываются в Этапах 3–4.
- [ ] **Документация обновлена** — черновик в `_docs/architecture.md` (финал — Этап 5).
- [ ] **Тесты добавлены / обновлены** — да.
- [ ] `git status` чист.
...
- [ ] `tests/adapters/max/test_messages.py`: входящий текстовый апдейт → вызов `handle_user_task` (мок) → `send_message` (мок); события `MessageReceived`/`ResponseGenerated` опубликованы с `channel="max"`.
- [ ] Тест на разбиение длинного ответа и на маппинг ошибок LLM в подсказки.
- [ ] **Документация обновлена** — черновик `_docs/architecture.md` §4 (MAX); финал — Этап 5.
- [ ] **Тесты добавлены / обновлены** — да; `pytest -q` зелёный.
- [ ] `git status` чист.
...
|-----|--------|:---------:|:-----:|:------:|:----------:|
| 1.1 | Актуализировать README под мульти-канальность и спринты 01–08 | high | S | Progress | — |
| 1.2 | Синхронизировать прочие документы и ссылки | medium | S | ToDo | 1.1 |
| 2.1 | Конфиг MAX и поддержка канала "max" | high | S | ToDo | — |
| 2.2 | MaxClient на httpx (get_me / get_updates / send_message) | high | M | ToDo | 2.1 |
| 2.3 | Polling-цикл и точка входа app/max_main.py | high | M | ToDo | 2.2 |
| 3.1 | Обработчик текста → core.handle_user_task | high | M | ToDo | 2.3 |
| 3.2 | Команды через общий CommandRegistry | high | M | ToDo | 3.1 |
