...
- [x] `README.md` и связанные документы актуализированы под состояние после спринтов 01–08: проект описан как мульти-агентная система с адаптерами Telegram + console, исправлена опечатка ссылки `./pp/skills` → `./app/skills`, список возможностей/спринтов соответствует `_docs/current-state.md`.
- [x] В `Settings` есть поля `MAX_*`; `.env.example` дополнен; `UserRepository.get_or_create("max", ...)` возвращает пользователя с `channel="max"`.
- [x] `MaxClient` (на `httpx`, без сторонних SDK) реализует `get_me` / `get_updates` / `send_message` / скачивание вложений; токен не попадает в логи (через `mask_secrets`); внешние вызовы логируются как `external.call/ok/fail`.
- [x] `app/max_main.py` собирается и запускает long polling; smoke `python -c "from app.max_main import main; print(main)"` проходит; graceful shutdown закрывает клиентов.
- [x] Текстовое сообщение из MAX проходит `sanitize_user_input` → `core.handle_user_task` → ответ в MAX; длинный ответ режется под лимит; публикуются `MessageReceived` / `ResponseGenerated` с `channel="max"`.
- [x] Команды (`/start`, `/help`, `/new`, `/reset`, `/models`, `/model`, `/prompt`, `/mode`, `/search_engines`, `/search_engine`) работают через общий `CommandRegistry`.
- [x] Документ / фото / голосовое из MAX скачиваются с проверкой размера и изоляцией по `user_id`, маршрутизируются в существующий конвейер (`read_document` / `Vision` / `Transcriber`); при недоступности faster-whisper / vision — корректные fallback-сообщения.
- [x] `README.md`, `_docs/roadmap.md` (Этап 6 снят), `_docs/current-state.md`, `_docs/architecture.md`, `_docs/project-structure.md` обновлены под MAX-адаптер; «битых» ссылок нет.
- [x] `pytest -q` и `flake8 app tests` зелёные; для задач, меняющих код в `app/`, есть unit-тесты на новое поведение (моки `httpx`, без сети).
...
- [x] `tests/adapters/max/test_commands.py`: `/help` и `/mode` проходят через MAX-диспетчер (реальный `CommandRegistry`) и возвращают ответ; неизвестная команда → подсказка.
- [x] Команда `/new` корректно вызывает архивирование с прогресс-коллбэком (как в console).
- [x] **Документация обновлена** — черновик `_docs/architecture.md` §3.13 (команды MAX); поведение команд идентично console/telegram, поэтому `_docs/commands.md` правок не требует; финал — Этап 5.
- [x] **Тесты добавлены / обновлены** — да.
- [x] `git status` чист.
...
- [x] `tests/adapters/max/test_files.py`: скачивание с замоканным `httpx` (`MockTransport`); превышение лимита по `Content-Length` и в потоке → `FileTooLargeError`; файл сохраняется в подкаталог пользователя.
- [x] Путь файла не выходит за `tmp_base_dir` (тест на изоляцию).
- [x] **Документация обновлена** — черновик `_docs/architecture.md` §6.6 (MAX); финал — Этап 5.
- [x] **Тесты добавлены / обновлены** — да.
- [x] `git status` чист.
...
- [x] `tests/adapters/max/test_file_messages.py`: три типа вложений с моками `download`/`Vision`/`Transcriber` → вызов `handle_user_task` с корректным goal; неподдерживаемый тип → подсказка.
- [x] Fallback-сообщения при недоступности vision / faster-whisper и при `FileTooLargeError` покрыты тестом.
- [x] **Документация обновлена** — черновик `_docs/architecture.md` §6.6 (MAX); финал — Этап 5.
- [x] **Тесты добавлены / обновлены** — да; `pytest -q` зелёный.
- [x] `git status` чист.

> **Примечание (сверка с API, 2026-06-05):** входящие вложения MAX содержат прямую ссылку `attachment.payload.url` на CDN (`fu.oneme.ru` / `iu.oneme.ru` / `vu.okcdn.ru`); скачивание — обычный потоковый `GET` без авторизации в query. Reply-контекст файлов MAX в MVP не используется (зафиксировать ограничение в `current-state.md` §2 на Этапе 5).
...
- [x] `README.md` описывает запуск MAX-канала (`python -m app.max_main`) и требование `MAX_BOT_TOKEN`; вводный абзац подчёркивает мульти-канальность/мульти-моделность/мульти-агентность.
- [x] `_docs/roadmap.md` больше не содержит Этап 6 «MAX-адаптер» как Backlog; этапы 7–15 перенумерованы в 6–14, ссылки согласованы; MAX-webhook упомянут в новом Этапе 6.
- [x] **Документация обновлена** — да.
...
- [x] `_docs/current-state.md` §1.4 (подсистема MAX-адаптера) и §2.2 (ограничения MVP), `_docs/architecture.md` (§3.13/§6.6/§11 финализированы), `_docs/project-structure.md` (дерево + таблица + тесты) отражают фактический код MAX-адаптера.
- [x] `.env.example` содержит финальный блок `MAX_*` (добавлен в задаче 2.1); `requirements.txt` без лишних зависимостей (`httpx` уже присутствует).
- [x] `grep` по ссылкам не находит «битых» путей; навигация `_docs/README.md` согласована.
- [x] **Документация обновлена** — да.
- [x] **Тесты добавлены / обновлены** — `n/a`.
- [x] `git status` чист.
...
| 2 | Long polling в MAX «не для production» (по их докам). | MVP осознанно на polling (как Telegram, CON-4); webhook вынесен в `roadmap.md` Этап 6 (non-goal). |
...
| 3.2 | Команды через общий CommandRegistry | high | M | Done | 3.1 |
| 4.1 | Скачивание вложений MAX с изоляцией по пользователю | medium | M | Done | 2.2 |
| 4.2 | Маршрутизация документов / фото / голоса в конвейер | medium | M | Done | 4.1, 3.1 |
| 5.1 | README + roadmap + plan под MAX | high | S | Done | 3.2, 4.2 |
| 5.2 | current-state + architecture + project-structure + .env.example | high | M | Done | 5.1 |

> Обновляется при каждом переходе статуса и при добавлении/удалении задач.

## 11. История изменений спринта

- **2026-06-05** — закрыты задачи 5.1 и 5.2 (Этап 5, документация): `README.md` переписан под мульти-канальность (Telegram + console + MAX) / мульти-моделность / мульти-агентность, добавлены запуск `python -m app.max_main` и требование `MAX_BOT_TOKEN`; `_docs/roadmap.md` — снят Этап 6 «MAX-адаптер», этапы перенумерованы (MAX-webhook в новом Этапе 6); `_docs/current-state.md` §1.4/§2.2, `_docs/architecture.md` §3.13/§6.6/§11, `_docs/project-structure.md` финализированы под MAX. Сверка с `dev.max.ru/docs-api`: подтверждены `Authorization: <token>`, `POST /messages` (user_id/chat_id, text ≤ 4000), long polling не для production. `pytest`/`flake8` зелёные. **Этап 5 закрыт, спринт закрыт.**
- **2026-06-05** — закрыты задачи 4.1 и 4.2 (Этап 4): `download_max_file` (`app/adapters/max/files.py`) — потоковое скачивание по `payload.url` с лимитом `MAX_MAX_FILE_MB` и изоляцией по `user_id`; `MaxUpdateDispatcher._handle_attachments` маршрутизирует `file`/`image`/`audio` в `read_document`/`Vision`/`Transcriber` → `core.handle_user_task`, неподдерживаемые типы и fallback'и покрыты; тесты `tests/adapters/max/test_files.py`, `tests/adapters/max/test_file_messages.py`; черновик `_docs/architecture.md` §6.6. `pytest`/`flake8` зелёные. **Этап 4 закрыт.**
- **2026-06-04** — спринт открыт, ветка `feature/09-max-adapter` создана от `main`.
- **2026-06-05** — закрыта задача 3.2: команды MAX через общий `CommandRegistry` с `CommandContext(channel="max")`, `/new` с прогресс-коллбэком, неизвестная команда → подсказка; тесты `tests/adapters/max/test_commands.py` (коммит `feat(max): команды через общий CommandRegistry`). **Этап 3 закрыт.**
