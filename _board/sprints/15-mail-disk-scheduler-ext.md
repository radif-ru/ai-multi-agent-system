...
- **2026-07-25** — постфактум: бейдж покрытия в README заменён со статического shields.io на динамический SVG из orphan-ветки `coverage-badge` (только `coverage.svg`, без остального репозитория). CI `test.yml`: job renamed `flake8 + pytest` → `flake8 + pytest + gates + coverage`, fix `percent_covered` из `totals` (не корень JSON), orphan-ветка через `git checkout --orphan` + `/tmp`, `coverage.json` добавлен в `.gitignore`. `_docs/testing.md` §5 обновлён: фактическое покрытие ~88%, описание динамического бейджа.
