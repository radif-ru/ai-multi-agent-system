...
└── skills/                   # скиллы-инструкции (<name>/SKILL.md)
    └── <name>/scripts/       # опционально: детерминированные скрипты скилла (.py/.sh)
```
...
| `review-loop.prompt.md` | Цикл «ревью → доработка» до чистого прохода с сортировкой замечаний. |
...
| `debugging-discipline` | Дисциплина отладки: стоп-линия, воспроизводящий тест, первопричина, минимальный фикс. |
| `skill-authoring` | Как добавлять скиллы/промпты ассистента: формат, зеркала, скрипт `check_agents_sync.py`. |

### Скрипты в скиллах (`skills/<name>/scripts/`)

Детерминированная часть скилла оформляется исполняемым скриптом, а не инструкцией для ИИ (принцип `automation-discipline`) — по аналогии со скриптами runtime-скиллов бота (`_docs/skills.md`), но без sandbox-раннера: среда разработки доверенная, скрипты запускаются напрямую из корня репозитория. Конвенция: только `.py`/`.sh`, без побочных эффектов (проверки и чтение), выход 0 = успех, ненулевой = проблема с деталями; каждый скрипт описан в теле своего `SKILL.md` (см. `skill-authoring`).

| Скрипт | Назначение |
|--------|------------|
| `git-discipline/scripts/preflight.sh` | Весь ритуал проверок перед коммитом одной командой: `flake8`, `pytest`, все скрипты-гейты. |
| `skill-authoring/scripts/check_agents_sync.py` | Проверка формата скиллов/промптов и зеркал (`.claude/skills/`, `AGENTS.md`, таблицы этого README). |
...
- **Скрипт скилла:** положить в `skills/<name>/scripts/`, описать в теле `SKILL.md` и в таблице скриптов выше (подробности — скилл `skill-authoring`).
- **Проверка:** `python3 .agents/skills/skill-authoring/scripts/check_agents_sync.py` — зелёный скрипт подтверждает, что формат и все зеркала на месте.
