...
# --- read_file лимит размера (спринт 12, задача 3.1) ---


def test_read_file_max_bytes_default(base_env):
    s = _build(base_env)
    assert s.read_file_max_bytes == 1024 * 1024


def test_read_file_max_bytes_from_env(base_env):
    s = _build(base_env, READ_FILE_MAX_BYTES=2048)
    assert s.read_file_max_bytes == 2048
