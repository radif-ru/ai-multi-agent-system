...
- **YandexDiskReader** — `app/services/yandex_disk.py` — read-only доступ к Яндекс.Диску по REST (`https://cloud-api.yandex.net/v1/disk`, заголовок `Authorization: OAuth <token>`) на общем `httpx.AsyncClient`. `list_path(path, limit)` (метаданные `/resources`), `download(path)` (ссылка `/resources/download` → скачивание в `Settings.get_user_tmp_dir(user_id)` с лимитом `TELEGRAM_MAX_FILE_MB`). Ошибки `DiskNotConfigured` / `DiskAuthError` / `DiskUnavailable`. Логи `service="yandex_disk"`, токен маскируется.
- **disk_list / disk_download** — `app/tools/disk_list.py`, `app/tools/disk_download.py` — tools реестра. `disk_list` (args: `path`) → JSON-список файлов/папок; `disk_download` (args: `path`) скачивает файл в каталог пользователя и возвращает file-id через `FileIdMapper`, что делает его читаемым существующим `read_document`.
