...
- **Error tracking через GlitchTip / Sentry** — `app/observability/__init__.py::setup_sentry` (off-by-default: при пустом `SENTRY_DSN` ничего не инициализируется). Разделение потоков (Спринт 14): ошибки уровня `SENTRY_EVENT_LEVEL` (по умолчанию `ERROR`) и выше уезжают в **Issues**; логи уровня `SENTRY_LOG_LEVEL` (по умолчанию `INFO`) — во вкладку **Logs** (`enable_logs=True`, `auto_session_tracking=False`; отключается `SENTRY_ENABLE_LOGS=false`); `DEBUG` включается через `SENTRY_LOG_LEVEL=DEBUG`. Performance-трассировки — `SENTRY_TRACES_SAMPLE_RATE` (дефолт `0.1`). `_before_send` подмешивает `trace_id`/`user_id` в `tags`/`extra`/`user`. Self-hosted GlitchTip разворачивается через `docker-compose.observability.yml` (postgres + redis + web/worker/migrate). См. `_docs/observability.md` §5.
