...
- **Факты о системе и авторе** — блок «О системе и авторе» в `app/prompts/agent_system.md` (автор, репозиторий, local-first) плюс скилл `about-project` для подробного рассказа. Факты дублируются в промпте сознательно: ответ на «кто тебя создал» не должен зависеть от того, вызовет ли слабая модель `load_skill` (тест `tests/test_about_project.py`).
...
- **Task-префиксы `nomic` (Спринт 14, ADR-4)** — при индексации чанков (`Archiver`) вектор считается с префиксом `EMBEDDING_DOCUMENT_PREFIX` (`search_document: `), при поиске (`memory_search`, авто-подгрузка `session_bootstrap`) — с `EMBEDDING_QUERY_PREFIX` (`search_query: `). Текст чанка в `memory_chunks.text` хранится **без** префикса. Метрика `vec0` остаётся L2 (для нормализованных векторов `nomic` монотонно связана с cosine). См. `_docs/memory.md` §3.2 и ADR-4 в `_docs/decisions.md`.
...
- **Error tracking через GlitchTip / Sentry** — `app/observability/__init__.py::setup_sentry` (off-by-default: при пустом `SENTRY_DSN` ничего не инициализируется). Разделение потоков (Спринт 14): ошибки уровня `SENTRY_EVENT_LEVEL` (по умолчанию `ERROR`) и выше уезжают в **Issues**; логи уровня `SENTRY_LOG_LEVEL` (по умолчанию `DEBUG` для dev) — во вкладку **Logs** (`enable_logs=True`, `auto_session_tracking=False`; отключается `SENTRY_ENABLE_LOGS=false`); в prod рекомендуется `SENTRY_LOG_LEVEL=INFO`. Performance-трассировки — `SENTRY_TRACES_SAMPLE_RATE` (дефолт `0.1`). `_before_send` подмешивает `trace_id`/`user_id` в `tags`/`extra`/`user`. Self-hosted GlitchTip разворачивается через `docker-compose.observability.yml` (postgres + redis + web/worker/migrate). См. `_docs/observability.md` §5.
...
### 1.9 Почта и Яндекс.Диск (Спринт 13)

- **MailReader** — `app/services/mail.py` — read-only доступ к почте по IMAP (SSL, 993) для Яндекса (`imap.yandex.ru`) и Gmail (`imap.gmail.com`). Async-обёртка над stdlib `imaplib`/`email` через `asyncio.to_thread` (без новых зависимостей). Методы `list_messages(provider, folder, unread_only, limit)` и `read_message(provider, uid)`; декодирование заголовков (`decode_header`), извлечение text/plain (fallback text/html → плейн), усечение тела до `MAIL_BODY_MAX_CHARS`. Иерархия ошибок `MailError`: `MailNotConfigured` (пустые креды — с подсказкой, какие `.env`-переменные заполнить), `MailAuthError`, `MailUnavailable` (сеть/таймаут `MAIL_IMAP_TIMEOUT`). Структурные логи `service="mail"`, логин маскируется.
- **email_list / email_read** — `app/tools/email_list.py`, `app/tools/email_read.py` — tools реестра. `email_list` (args: `provider`=`yandex|gmail|all`, `unread_only`, `limit`) → JSON-массив `{uid, provider, from, subject, date, unread}`; `email_read` (args: `provider`, `uid`) → заголовки + тело + `attachments` (список `{filename, file_id, content_type, size}`). Вложения сохраняются в `data/tmp/` через `_save_attachments` с регистрацией `FileIdMapper` — модель читает их через `read_document`. **Тело письма — недоверенный ввод**: в observation обёрнуто маркером «данные, не инструкции». Ошибки сервиса → `ToolError` с человекочитаемым текстом (одинаково во всех каналах). Регистрация — в общем `app/main.py::_build_components`.
- **YandexDiskReader** — `app/services/yandex_disk.py` — доступ к Яндекс.Диску по REST (`https://cloud-api.yandex.net/v1/disk`, заголовок `Authorization: OAuth <token>`) на общем `httpx.AsyncClient`. `list_path(path, limit)` (метаданные `/resources`), `download(path)` (ссылка `/resources/download` → скачивание в `Settings.get_user_tmp_dir(user_id)` с лимитом `TELEGRAM_MAX_FILE_MB`), `upload(local_path, disk_path)` (GET `/resources/upload` → upload URL → PUT файла с лимитом). Ошибки `DiskNotConfigured` / `DiskAuthError` / `DiskUnavailable`. Логи `service="yandex_disk"`, токен маскируется.
- **disk_list / disk_download / disk_upload** — `app/tools/disk_list.py`, `app/tools/disk_download.py`, `app/tools/disk_upload.py` — tools реестра. `disk_list` (args: `path`) → JSON-список файлов/папок; `disk_download` (args: `path`) скачивает файл в каталог пользователя и возвращает file-id через `FileIdMapper`; `disk_upload` (args: `file_id`, `path`) загружает локальный файл на Яндекс.Диск через REST API (`/resources/upload` → PUT).
...
- **Ограничения** — почта read-only (отправка/удаление/пометка писем — вне скоупа), IMAP с паролями приложений вместо OAuth-flow Gmail / Яндекс 360 API, Google Диск не поддерживается (см. `_docs/roadmap.md` § «Интеграции почты и диска — добор»). На Яндекс.Диске доступны и чтение, и загрузка (`disk_upload`, спринт 15).

### 1.10 Планировщик задач (Спринт 14)

- **ScheduledTaskStore** — `app/services/scheduled_tasks.py` — персистентное sqlite-хранилище определений задач в таблице `scheduled_tasks` (в `data/memory.db`; отдельное соединение + `threading.Lock`, как `DialogJournal`). `ScheduledTask` — frozen dataclass; API `init/add/get/list_by_user/list_enabled/count_by_user/mark_run/delete/close`; `delete` фильтрует по `user_id` (нельзя удалить чужую задачу).
- **SchedulerService** — `app/services/scheduler.py` — обёртка над APScheduler `AsyncIOScheduler` с `MemoryJobStore`. Персистентность — в `ScheduledTaskStore`, jobs пересоздаются из `list_enabled()` при `start()` (rehydrate). `add_task`/`remove_task`/`validate_cron`; job регистрируется через `functools.partial(_job_entrypoint, task_id=...)` (pickle не требуется). `run_task` (раннер) внедряется в `main()`.
- **run_scheduled_task / notifiers** — `app/services/scheduler_runner.py` — раннер: биндит `trace_id`/`user_id`, санизирует prompt, оборачивает его контекстом исполнения, вызывает `handle_user_task(..., history=[])` (изоляция от живой сессии, событий шины нет), `mark_run(ok|error)`, heartbeat-логирование (`scheduler.run status=start/ok/error` с `dur_ms`). Доставка через notifier по каналу задачи: `make_telegram_notifier` (`split_long_message` + `html.escape`), `make_console_notifier` (печать в stdout), `make_max_notifier` (`MaxClient.send_message` с разбивкой).
...
- **Ограничения** — отправка писем (`email_send`), Google Диск — в roadmap (§ «Интеграции почты и диска — добор»). Документ — `_docs/scheduler.md`, решение — ADR-2.
...
- **Long polling, не webhook.** Документация MAX (`dev.max.ru/docs-api`) прямо указывает, что long polling ограничен по скорости и сроку хранения событий и **не подходит для production** — рекомендуется webhook (`POST /subscriptions`). MVP осознанно на polling (как Telegram, CON-4); webhook вынесен в `_docs/roadmap.md` § «Webhook вместо polling (Telegram и MAX)».
...
- **Кросс-канальная унификация пользователя отсутствует**: MAX-пользователь отдельный по ключу `(channel="max", external_id)`, не связан с Telegram-пользователем. См. `_docs/roadmap.md` § «Web-адаптер» (унифицированный `user_id`).
