...
- **run_scheduled_task / make_telegram_notifier** — `app/services/scheduler_runner.py` — раннер: биндит `trace_id`/`user_id`, санизирует prompt, оборачивает его контекстом исполнения, вызывает `handle_user_task(..., history=[])` (изоляция от живой сессии, событий шины нет), `mark_run(ok|error)`, `_cron_checkin` в GlitchTip Crons, доставка в Telegram через notifier (`split_long_message` + `html.escape`).
...
- **Скилл** — `app/skills/scheduler/SKILL.md` — маппинг естественного языка в 5-польный cron (LLM делает маппинг, парсера времени в коде нет).
...
- **Ограничения MVP** — доставка только в Telegram (console/MAX → `_docs/roadmap.md`); cron-only, без bot-команд `/schedule` и без парсера естественного времени в коде. Документ — `_docs/scheduler.md`, решение — ADR-2.
