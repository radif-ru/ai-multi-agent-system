...
[Graphify](https://github.com/safishamsi/graphify) — инструмент для построения графа кода. Используется для навигации по зависимостям и структуре проекта. Граф генерируется автоматически при коммитах через git hook (`graphify hook install`).
...
`graphweave` (синхронизация графов нескольких репозиториев) — **опционален**, нужен только для монорепо с субмодулями. В текущем проекте — single repo, достаточно одного `graphify`.

Первый запуск — в чате IDE: `/graphify .` (скилл настройки, если доступен).

### Использование

| Команда | Где запускать | Описание |
|---------|---------------|----------|
| `graphify update` | корень репо | Ручное обновление графа |
| `graphify hook install` | корень репо | Установить git-хук для авто-обновления при коммите |
| `graphify hook uninstall` | корень репо | Удалить git-хук |

Когда запускать `graphify update` вручную: после значительного рефакторинга, добавления/удаления модулей, изменения структуры `app/`. Хук обновляет граф автоматически при каждом коммите.

### Файлы

- `graphify-out/` — каталог с генерируемым графом (`graph.json`, `manifest.json`, `GRAPH_REPORT.md`, `.graphify_labels.json`). **В `.gitignore`**, в репозиторий не коммитится.
- `.graphifyignore` — исключения из графа (документы, медиа, конфиги, runtime-данные, каталоги документации и процесса). Code-only graph: нетекстовые форматы (PDF, office, картинки) требуют LLM для семантической экстракции и исключены, чтобы `graphify update` работал без API-ключа.

## 15. Чего в стеке нет (и не будет в MVP)
