...
Консольный адаптер — «эталонный» пример реализации адаптера для работы с агентом без Telegram. Демонстрирует минимальный контракт адаптера с `core.handle_user_task` и может служить шаблоном для новых адаптеров (например, web; по тому же принципу сделан MAX-адаптер).
...
| `/search_engine` | Переключить активный поисковик.                          |
| `/new`       | Архивировать сессию и открыть новую.                        |
| `/reset`     | Очистить in-memory историю + сбросить per-user настройки.    |
| `/schedule`  | Создать запланированную задачу (cron + prompt).              |
| `/schedules` | Список запланированных задач пользователя.                   |
...
- `_docs/architecture.md` §8.4 — новый адаптер (console, web, MAX).
