...
- **Динамический бейдж:** CI (`test.yml`) при пуше в `main` генерирует SVG-бейдж с фактическим процентом покрытия и пушит его в orphan-ветку `coverage-badge` (только `coverage.svg`, без остального репозитория). README ссылается на `raw.githubusercontent.com/.../coverage-badge/coverage.svg` — бейдж обновляется автоматически после merge в `main`, без ручного редактирования.
