...
| Что | Ссылка | Где в проекте |
|-----|--------|---------------|
| Graphify | https://github.com/safishamsi/graphify | Построение графа кода; установка через `uv tool install graphify`, см. `_docs/stack.md` §14 |
| uv | https://docs.astral.sh/uv/ | Менеджер Python-инструментов; используется для установки `graphify` (`uv tool install`) |
