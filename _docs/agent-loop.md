...
- Поля одного формата вперемешку с полями другого — допускается: если `final_answer` присутствует и непустой, парсер берёт его, игнорируя `thought`/`action`/`args` (модель часто добавляет «рассуждение для себя» рядом с готовым ответом). Если `final_answer` пустой — ответ трактуется как шаг с действием.
...
Маленькие модели иногда срываются с контракта: кладут рассуждение в `thought` и завершают объект как `action: null` или `action: "final_answer"`, не записав `final_answer`. Раньше «толерантные» ветки парсера подменяли `final_answer` текстом `thought` — и рассуждение модели утекало пользователю вместо ответа. Этого больше **не происходит**: `parse_agent_response` отдаёт наружу только поле `final_answer`, а срыв формата (включая `action: null`, `action == "final_answer"`, «только `thought`») → `LLMBadResponse`. Исключение: если модель вернула `final_answer` вместе с `thought` — парсер берёт `final_answer` и игнорирует `thought` (это не утечка, а спасение настоящего ответа).

Чтобы срыв формата не превращался сразу в ошибку, Executor делает ограниченную **само-починку**: при `LLMBadResponse` он дописывает в диалог исходный (невалидный) ответ модели и корректирующее сообщение с напоминанием формата (включая конкретный пример ошибки и правильного формата), после чего переспрашивает модель — до `AGENT_MAX_REPAIR_ATTEMPTS` раз (default `3`; `0` — выключить). Если хотя бы одна попытка вернула валидный JSON — цикл продолжается как обычно. Если попытки исчерпаны — поднимается `LLMBadResponse`, цикл прерывается, и пользователь получает нейтральное «Модель ответила в неожиданном формате» — но **никогда** внутреннее рассуждение модели.
...
|-------------|------------------------|
| **Лимит шагов** | `AGENT_MAX_STEPS` (default 30). По достижении — корректный выход с сообщением. |
...
| **Невалидный JSON / срыв формата** | `LLMBadResponse`. Сначала — до `AGENT_MAX_REPAIR_ATTEMPTS` (default 3) корректирующих переспросов (§2.4); после исчерпания цикл прерывается. `thought` не выдаётся за ответ. |
...
- **Стриминг шагов**: показывать пользователю «Шаг N: <thought>…» в виде edit'а исходящего сообщения (`roadmap.md` § «Stream-индикация шагов агента»).
...
- **Reflection-loop**: после `final_answer` запускать Critic; если он `REVISE` — возвращать в цикл с фидбеком (`roadmap.md` § «Capability graph для multi-agent»).
- **Динамический `AGENT_MAX_STEPS` по сложности задачи**: Planner оценивает сложность и предлагает лимит. Там же.
