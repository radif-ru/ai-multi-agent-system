...
- **Модульный монолит.** Один Python-процесс на канал, общий `core` с агентным циклом. Микросервисы отклонены для single-user local-first системы (ADR-3, `_docs/decisions.md`): сетевые хопы ухудшают латентность LLM-цикла, деплой усложняется. Эволюционный путь: FastAPI-адаптер (`_docs/roadmap.md` § «Webhook вместо polling (Telegram и MAX)») → docker-compose (§ «Docker / docker-compose») → микросервисы при многопользовательности.
...
- Сборка долгоживущих сервисов вынесена в `_build_components(...) -> _Components` (channel-agnostic). Консольная (`app/console_main.py`) и MAX (`app/max_main.py`) точки входа переиспользуют эту же функцию, добавляя только свой транспорт (`ConsoleAdapter` / `MaxClient`).
- Закрытие **общих** ресурсов (`llm`, `semantic_memory`, `dialog_journal`, `users`, `FileIdMapper`) вынесено в `_shutdown_components(components)`; каждая точка входа закрывает только свой транспорт (`bot.session` / `MaxClient`; у консоли транспорта нет) и вызывает общий хелпер.
...
- **OCR (по умолчанию включён, `DOCUMENT_OCR_ENABLED=true`):** если установлен tesseract-ocr:
  ...
  - Если OCR не справился — возвращается путь к картинке с подсказкой вызвать `ocr_image` или `describe_image`.
