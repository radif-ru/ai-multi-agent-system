...
> **Task-префиксы.** `nomic-embed-text` рекомендует префиксы `search_document:` (при индексации) и `search_query:` (при поиске). Внедрены в задаче 3.2 спринта 14 (см. [ADR-4](./decisions.md#adr-4-качество-rag-task-префиксы-метрика-чанкинг)). Конфиг: `EMBEDDING_DOCUMENT_PREFIX` / `EMBEDDING_QUERY_PREFIX` (пустая строка = отключить). Текст чанка в `memory_chunks.text` хранится **без** префикса — префикс добавляется только при вычислении вектора.

> **Метрика `vec0`.** `sqlite-vec` по умолчанию использует L2 (Euclidean). `nomic-embed-text` выдаёт нормализованные векторы, для которых L2 и cosine монотонно связаны — ранжирование идентично. Смена метрики отклонена (ADR-4).
