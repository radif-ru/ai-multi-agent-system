...
В будущих спринтах появятся tools, требующие настоящего sandboxing'а (например, выполнение shell-команд, чтение из произвольного места ФС, запись в БД). Они будут идти отдельным контрактом `SandboxedTool` (см. `roadmap.md` Этап 10).
