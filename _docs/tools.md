...
### 1.1 Зачем инструменты (а не «всё в LLM»)

Tools нужны там, где модели нельзя доверять «по памяти»: они дают **детерминизм, точность и свежие данные** вместо галлюцинаций. Разделение ответственности — LLM рассуждает и выбирает инструмент, tool выполняет проверяемую операцию:

- **Точность вместо галлюцинаций.** Арифметику считает `calculator`, а не LLM; текст со скана/чека извлекает OCR (Tesseract — `ocr_image`, OCR-ветка `read_document`), а не «угадывание».
- **Свежие/внешние данные.** `web_search` (`ddgs`), `http_request`, `weather` (wttr.in с фолбэком на веб-поиск) приносят то, чего нет в весах модели.
- **Доступ к состоянию проекта.** `memory_search` ищет по долгосрочной семантической памяти, `read_file` / `read_document` — по файлам, `load_skill` — по библиотеке навыков.
- **Разные модальности — разные средства.** Для изображений OCR (точная транскрипция текста) и vision-модель `describe_image` (описание сцены) **дополняют** друг друга, а не заменяют.
...
В будущих спринтах появятся tools, требующие настоящего sandboxing'а (например, выполнение shell-команд, чтение из произвольного места ФС, запись в БД). Они будут идти отдельным контрактом `SandboxedTool` (см. `roadmap.md` § «Sandboxed tools»).
